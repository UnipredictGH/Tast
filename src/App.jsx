  ...
  {tab === 'payment' && (
    <div className="max-w-sm">
      <Card className="p-5 space-y-3">
        <div className="font-bold text-[14px]">Paystack API Keys</div>
        <div className="text-[12px] text-gray-400">Only the public key is sent to the student app.</div>
        <div><label className="block text-[12px] font-semibold mb-1">Public Key (pk_…)</label><Input value={paystack.public_key || ''} onChange={v => setPaystack(p => ({ ...p, public_key: v }))} placeholder="pk_live_…" /></div>
        <div><label className="block text-[12px] font-semibold mb-1">Secret Key (sk_…)</label><Input value={paystack.secret_key || ''} onChange={v => setPaystack(p => ({ ...p, secret_key: v }))} placeholder="sk_live_…" type="password" /></div>
